...
// Metalama.Extensions.DiffEngine ships no lib/ folder: it is a build-time extension with no run-time API to call,
// so there is no feature to assert at run time. It plugs a diff tool into aspect-test failures.
//
...
internal static class Program
{
    public static int Main()
    {
        ...
        PackageTestCheck.That( result == 42, "the project referencing Metalama.Extensions.DiffEngine builds and runs" );
        PackageTestCheck.That( Marker.Called, "Metalama actually wove the compilation" );

        return PackageTestCheck.Report( "Metalama.Extensions.DiffEngine" );
    }
}
